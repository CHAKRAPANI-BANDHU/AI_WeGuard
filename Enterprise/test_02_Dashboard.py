...
def test_tc_2003_Dashboard_Recent_Activity_Logs_GET(Page, Size):
    now1 = datetime.now()
    if globalvar.bearerToken == '':
        pytest.skip("Empty Bearer token. Skipping test")
    try:
        apiURL = globalvar.BaseURL + DashboardRecentActivity(globalvar.activationCode, globalvar.productActivationCode, Page, Size, globalvar.OneWeekCustomISO, globalvar.isoend)
        Headers = {'Authorization': 'Bearer {}'.format(globalvar.bearerToken)}
        res = requests.get(url=apiURL, headers=Headers, timeout=globalvar.timeout)
        curl_str1 = Utils.getCurlEquivalent(res)
        print(curl_str1)
