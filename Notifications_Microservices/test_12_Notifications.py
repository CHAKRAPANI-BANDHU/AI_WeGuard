    ...
    try:
        for policyId in globalvariables.Android_profile_ids:
            ...
            if res.status_code == 200:
                ...
                print("All Geofence IDs:", Generalpayload.GeofenceIDS + "\n")
                print("All MongoDB IDs:", Generalpayload.Geofence_MongoDB_IDs + "\n")
                print("All Geofence Types:", Generalpayload.GeofenceTypes + "\n")
                print(
                    "\n--------------------------- GET method to get policy level Notifications ---------------------------\n")
            elif res.status_code == 400:
                print("\n" + "400 Bad Request!")
...
# POST method to get the time spent details in the Geofence
@pytest.mark.parametrize('url', [""])
@pytest.mark.skipif(Execute.test_1113_Notification_PolicyLevel_Timespent_Geofence_POST == 0, reason="Skip test")
@pytest.mark.positivetest
@pytest.mark.notifications
@pytest.mark.sanitytest
@pytest.mark.regressiontest
@pytest.mark.run(order=11013)
def test_tc_1113_POST_PolicyLevel_Timespent_Geofence(url):
    now1 = datetime.datetime.now()
    if globalvariables.bearerToken == '':
        pytest.skip("Empty Bearer token, Skipping test")
    try:
        for policyId in globalvariables.Android_profile_ids:
            for android_device_id in globalvariables.Android_DeviceIDs:
                for geofenceId in Generalpayload.GeofenceIds:
                    ...
                    Headers = {'Authorization': 'Bearer {}'.format(globalvariables.bearerToken)}
                    Generalpayload.TimespentGeofencePayload["policyId"] = policyId
                    Generalpayload.TimespentGeofencePayload["deviceIdList"] = android_device_id
                    Generalpayload.TimespentGeofencePayload["geofenceId"] = geofenceId
                    res = requests.get(url=apiUrl, headers=Headers, json=Generalpayload.TimespentGeofencePayload, timeout=globalvariables.timeout)
                    curl_str1 = Utils.getCurlEquivalent(res)
                    print(curl_str1)
                    if res.status_code == 200:
                        print("\n" + "200 The request was a success!")
