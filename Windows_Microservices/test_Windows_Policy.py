...
@pytest.mark.skipif(Execute.test_tc_00000010_Windows_Policy_Clone == 0, reason="skip test")
@pytest.mark.positivetest
@pytest.mark.WindowsDevice
@pytest.mark.regressiontest
@pytest.mark.run(order=100010)
def test_tc_0000010_Windows_Policy_Clone_POST(url):
    now1 = datetime.now()
    if globalvar.bearerToken == '':
        pytest.skip("Empty Bearer token Skipping test")
    try:
        for policy_id in globalvar.Windows_profile_ids:
            for policyType in globalvar.Windows_profile_type:
...
# @pytest.mark.parametrize('url', [""])
# @pytest.mark.skipif(Execute.test_tc_00000011_Windows_Policy_Delete == 0, reason="skip test")
# @pytest.mark.positivetest
# @pytest.mark.WindowsDevice
# @pytest.mark.regressiontest
# @pytest.mark.run(order=100011)
# def test_tc_0000011_Windows_Policy_DELETE(url):
#     now1 = datetime.now()
#     if globalvar.bearerToken == '':
#         pytest.skip("Empty Bearer token Skipping test")
#     try:
