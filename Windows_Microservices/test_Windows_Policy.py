...
def WindowsDevicePolicy(policyId, size, page):
    return "windows/rest/device/policy/{policyId}?size={size}&page={page}".format(policyId=policyId, size=size, page=page)
...
def test_tc_1100001_Windows_Policy_GET(url):
    ...
    try:
        for policyMongoID in globalvar.Windows_Policy_IDs:
            apiUrl = globalvar.BaseURL + GETPolicyByMongoID(policyMongoID)
